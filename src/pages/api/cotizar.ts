 ...
 * navegador) porque es el único lugar donde puede vivir la service role
 * key de Supabase: esa key ignora RLS, así que exponerla en el cliente
 * dejaría la tabla de cotizaciones abierta a cualquiera.
 *
 ...
 * falla, la cotización ya quedó guardada y se puede recuperar a mano.
 */
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!analisis.success) {
    return json({ ok: false, error: 'Datos inválidos.', detalles: analisis.error.issues }, 400);
  }
  const datos = analisis.data;

  // Honeypot: un campo que ningún humano llena porque está oculto por CSS.
  if (datos.sitioWeb) {
    return json({ ok: true }); // 200 falso para no darle señal al bot.
  }
  // Un envío completado antes de este umbral es casi con certeza un script.
  ...
  // --- Notificar al taller -------------------------------------------------
  // Un fallo acá no debe invalidar el envío: el dato ya está a salvo en la
    ...
    try {
      ...
      if (adjuntoPath) {
        const { data: firmada } = await supabaseAdmin.storage
          .from('adjuntos-cotizacion')
          .createSignedUrl(adjuntoPath, 60 * 60 * 24 * 7); // 7 días
      ...
      await resend.emails.send({
        ...
        to: [EMAIL_TALLER],
        replyTo: datos.email,
        subject: `Nueva cotización: ${datos.nombre} — ${datos.anchoCm}×${datos.altoCm} cm`,
        html: `
          <h2>Nueva solicitud de cotización</h2>
          <p><b>Nombre:</b> ${escapar(datos.nombre)}<br>
             <b>Email:</b> ${escapar(datos.email)}<br>
             <b>Teléfono:</b> ${escapar(datos.telefono || '—')}<br>
             <b>Tipo de cliente:</b> ${escapar(datos.tipoCliente)}<br>
             <b>Comuna:</b> ${escapar(datos.comuna || '—')}</p>
          <p><b>Aplicación:</b> ${escapar(datos.aplicacion || '—')}<br>
             <b>Medidas:</b> ${datos.anchoCm} × ${datos.altoCm} cm &times; ${datos.cantidad}<br>
             <b>Ubicación:</b> ${escapar(datos.ubicacion)}<br>
             <b>Patrón:</b> ${escapar(datos.patron || 'a recomendar')}<br>
             <b>Material:</b> ${escapar(datos.material || 'a recomendar')}<br>
             <b>Terminación:</b> ${escapar(datos.terminacion || '—')}<br>
             <b>Plazo:</b> ${escapar(datos.plazo || '—')}</p>
          ${datos.mensaje ? `<p><b>Mensaje:</b><br>${escapar(datos.mensaje)}</p>` : ''}
          ${urlAdjunto ? `<p><a href="${urlAdjunto}">Ver adjunto</a> (enlace válido 7 días)</p>` : ''}
          <p style="color:#888;font-size:12px">ID interno: ${fila.id}</p>
        `,
      });
    } catch {
      // Se ignora a propósito: la cotización ya está guardada en Supabase.
    }
  }
